...
char train_file[MAX_STRING], output_file[MAX_STRING], output_posfile[MAX_STRING], temp_file[MAX_STRING];
...
int SearchVocab(char *word, long pos, int flag) {
...
int ReadWordIndex(FILE *fin) {
  ...
  return SearchVocab(word,ftell(fin),0);
}
...
void TrainModel() {
  ...
  if (classes == 0) {
	...
	for(it=wordpos.begin();it!=wordpos.end();it++){
		fprintf(fo2, "%s ", (it->first).c_str());
		int vsize = it->second.size();
		for(int i = 0; i < vsize; i++){
			fprintf(fo2, "%d ", it->second[i]);
		}
		fprintf(fo2, "\n");
	}
  } else {
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-outputpos", argc, argv)) > 0) strcpy(output_posfile, argv[i + 1]);
